...
public class EnhancedGameTest implements FabricGameTest {
    ...
    public void moreMendingBetterMendingCombined(TestContext helper) {
        ...
        net.minecraft.entity.ExperienceOrbEntity orb = new net.minecraft.entity.ExperienceOrbEntity(world, pos.getX(), pos.getY(), pos.getZ(), 100);
        try {
            // betterMending takes over repairPlayerGears and must set mendingLevel=5 before the cost call
            int remaining = ETTestHelper.repairPlayerGears(orb, player, 100);
            // level-5 factor 0.35 -> cost 14 -> remaining 86. Old level-0 worst-case (0.6) -> cost 24 -> remaining 76
            helper.assertTrue(remaining == 86,
                "Combined MoreMending+BetterMending should use the item's actual Mending level (remaining=" + remaining + ", expected 86; the old level-0 bug would give 76)");
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
            ETTestHelper.setFeature("more_mending", false);
    ...
    public void moreMendingFloorClampNoCrash(TestContext helper) {
        ...
        try {
            ...
            getMRC.setAccessible(true);
            boolean isStatic = java.lang.reflect.Modifier.isStatic(getMRC.getModifiers());
            // pre-fix this throws IllegalArgumentException (surfaced as InvocationTargetException)
            int cost = (int) getMRC.invoke(isStatic ? null : orb, 100);
            // floor clamped to 0.6 -> factor = clamp(0.6 - 0.05*0, 0.6, 0.6) = 0.6 -> round(100*0.6) = 60
            helper.assertTrue(cost == 60,
                "MoreMending with floor=0.9 (>0.6) should clamp the floor to 0.6 and return 60 without crashing (got " + cost + ")");
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        } finally {
            ETTestHelper.setFeature("more_mending", false);
    ...
    public void moreLootingEnabled(TestContext helper) {
        ...
        try {
            ...
            // looting 3, mult 0.5 -> factor 2.5, applied once to the whole base: (int)(10 * 2.5) = 25
    ...
    public void moreLootingDisabled(TestContext helper) {
        ...
        try {
            int base = 10;
            int total = dropAndSumXp(helper, world, zombie, player, base);
            // feature disabled: the mixin returns xp unchanged, so the orb total is exactly the base
    ...
    // math.max(0.0, multiplier): a negative multiplier must not reduce XP below the base amount
    ...
    public void moreLootingNegativeMultiplier(TestContext helper) {
        ...
        try {
            int base = 10;
            int total = dropAndSumXp(helper, world, zombie, player, base);
            // math.max(0.0, mult) clamps -1.0 -> 0 -> factor 1.0 -> XP unchanged (a boost never reduces XP)
    ...
    // math.clamp(base, 0.0, 1.0): a negative base becomes 0 -> damage * 0^epf = 0, never NaN
    ...
    // math.max(0, perLevel) then clamp(..., 1, MAX): a negative per_level yields exactly 1 arrow
    ...
    private static void forceThunderGradient(ServerWorld world, float gradient) {
        try {
            for (String name : new String[]{"thunderGradient", "thunderGradientPrev", "rainGradient", "rainGradientPrev"}) {
    ...
    /** equips a full diamond armor set on {@code entity}, each piece carrying {@code ench} at {@code level} */
    private static void equipArmorSet(net.minecraft.entity.LivingEntity entity, net.minecraft.enchantment.Enchantment ench, int level) {
        for (EquipmentSlot slot : new EquipmentSlot[]{EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET}) {
            ItemStack armor = new ItemStack(switch (slot) {
                case HEAD -> Items.DIAMOND_HELMET;
                case CHEST -> Items.DIAMOND_CHESTPLATE;
    ...
    private static int dropAndSumXp(TestContext helper, ServerWorld world, ZombieEntity zombie, ServerPlayerEntity killer, int baseXp) {
        try {
            ...
            // tight box on the mob itself; orbs spawn at zombie.getPos() and never drift (no tick)
            ...
            return world.getEntitiesByClass(ExperienceOrbEntity.class, searchBox, e -> true)
    ...
    public void moreBindingAfterRespawnRestore(TestContext helper) {
        ...
        try {
            ...
            // keep path: the mixin makes isEmpty() report true so vanilla skips the drop; the helmet
            ...
            helper.assertTrue(restored.isOf(Items.DIAMOND_HELMET)
    ...
    public void moreFlameEndToEndRealShot(TestContext helper) {
        ...
        try {
            ...
            List<PersistentProjectileEntity> arrows = world.getEntitiesByClass(PersistentProjectileEntity.class, search, e -> true);
    ...
    public void moreInfinityCreativeNoConsume(TestContext helper) {
        ...
        try {
            ...
            helper.assertTrue(player.getInventory().count(Items.ARROW) == arrowsInvBefore,
                "Creative shot must not consume inventory arrows (before=" + arrowsInvBefore + ", after=" + player.getInventory().count(Items.ARROW) + ")");
            ...
            for (int i = 0; i < 20; i++) {
                ...
                helper.assertTrue(player.getInventory().count(Items.ARROW) == base,
                    "Creative Infinity shots must never consume arrows (iteration " + i + ", got " + player.getInventory().count(Items.ARROW) + ")");
            }
        } finally {
            ETTestHelper.setFeature("more_infinity", false);
            ETTestHelper.setConfigValue("more_infinity_pct", "0.03");
    ...
    public void moreProtectionUncappedEpf(TestContext helper) {
        ...
        try {
            ...
            helper.assertTrue(Math.abs(r32 - exp32) < 0.05f,
                "EPF 32 should give ~" + exp32 + " (got " + r32 + ")");
            ...
            helper.assertTrue(Math.abs(r40 - exp40) < 0.05f,
                "EPF 40 should give ~" + exp40 + " (got " + r40 + ")");
            helper.assertTrue(r40 < r32,
                "EPF 40 must deal strictly less damage than EPF 32 (scaling past the vanilla 20 cap): r40=" + r40 + ", r32=" + r32);
        } finally {
            ETTestHelper.setConfigValue("more_protection_base", "0.96");
            ETTestHelper.setFeature("more_protection", false);
        }
    ...
    // orig=isThundering()=true so lightning summons regardless of level; verifies the mixin does not
    ...
    public void moreInfinityRealPathFreeConsume(TestContext helper) {
        ...
        try {
            ...
            helper.assertTrue(player.getInventory().count(Items.ARROW) == freeBefore,
                "FREE shot (pct=1.0) must not consume an arrow (before=" + freeBefore + ", after=" + player.getInventory().count(Items.ARROW) + ")");
            ...
            helper.assertTrue(player.getInventory().count(Items.ARROW) == consumeBefore - 1,
                "CONSUME shot (pct=0.0) must consume exactly one arrow (before=" + consumeBefore + ", after=" + player.getInventory().count(Items.ARROW) + ")");
        } finally {
            ETTestHelper.setConfigValue("more_infinity_pct", "0.03");
            ETTestHelper.setFeature("more_infinity", false);
        }
    ...
    public void moreMendingLevelScalingRealPath(TestContext helper) {
        ...
        try {
            ...
            for (int idx = 0; idx < levels.length; idx++) {
                ...
                int remaining = ETTestHelper.repairPlayerGears(orb, player, 100);
                helper.assertTrue(remaining == expected[idx],
                    "MoreMending level " + levels[idx] + " should leave " + expected[idx] + " XP (got " + remaining + ")");
                helper.assertTrue(sword.getDamage() == 0,
                    "MoreMending level " + levels[idx] + " should fully repair the sword (damage=" + sword.getDamage() + ")");
            }
        } finally {
            ETTestHelper.setFeature("more_mending", false);
            ETTestHelper.setFeature("better_mending", false);
    ...
    // floor=-0.5 clamps to 0.0; at level 12 factor = clamp(0.6-0.6, 0.0, 0.6) = 0.0 -> cost 0 -> all XP kept
    ...
    // step=0 -> factor = clamp(0.6 - 0, 0.1, 0.6) = 0.6 -> cost round(100*0.6)=60 -> remaining 40
    ...
    public void moreMendingMultiItemRecursion(TestContext helper) {
        ...
        try {
            ...
            helper.assertTrue(helmet.getDamage() == 0 && sword.getDamage() == 0,
                "Both Mending items should be fully repaired (helmet=" + helmet.getDamage() + ", sword=" + sword.getDamage() + ")");
        } finally {
            ETTestHelper.setFeature("more_mending", false);
            ETTestHelper.setFeature("better_mending", false);
            ETTestHelper.setConfigValue("more_mending_step", "0.05");
    ...
    // 4 pieces each carry PROTECTION 4 (ALL -> 4) and BLAST_PROTECTION 4 (EXPLOSION -> 4*2=8) -> EPF 48
    ...
    public void moreProtectionGodArmorEpf48(TestContext helper) {
        ...
        net.minecraft.entity.damage.DamageSource source =
            helper.getWorld().getDamageSources().explosion((net.minecraft.world.explosion.Explosion) null);
        float result = ETTestHelper.modifyAppliedDamage(entity, source, 20.0f);
    ...
    public void moreFlameNullOwner(TestContext helper) {
        ...
        try {
            ...
            onEntityHit.invoke(arrow, new EntityHitResult(target));
            helper.assertTrue(target.getFireTicks() == 100,
                "An on-fire arrow with a null owner (flameLevel 0) should burn vanilla 100 ticks (got " + target.getFireTicks() + ")");
        } catch (ReflectiveOperationException e) {
            throw new RuntimeException(e);
        } finally {
            ETTestHelper.setConfigValue("more_flame_per_level", "2");
    ...
    public void moreFireProtectionNegativeBase(TestContext helper) {
        ...
        try {
            ...
            helper.assertTrue(result == 0,
                "Negative base clamps to 0 -> duration 0 (got " + result + ")");
        } finally {
            ETTestHelper.setConfigValue("more_fire_protection_base", "0.85");
            ETTestHelper.setFeature("more_fire_protection", false);
        }
    ...
    public void moreFireProtectionUpperBase(TestContext helper) {
        ...
        try {
            int result = net.minecraft.enchantment.ProtectionEnchantment.transformFireDuration(entity, 200);
            helper.assertTrue(result == 200,
                "base > 1 clamps to 1 -> duration unchanged at 200 (got " + result + ")");
        } finally {
            ETTestHelper.setConfigValue("more_fire_protection_base", "0.85");
            ETTestHelper.setFeature("more_fire_protection", false);
        }
    ...
    public void moreLootingZeroLevelGuard(TestContext helper) {
        ...
        try {
            int base = 10;
            int total = dropAndSumXp(helper, world, zombie, player, base);
            // `lootingLevel` <= 0 guard: the mixin returns xp unchanged even with the feature on
    ...
    public void moreInfinityNonArrowProjectileConsumed(TestContext helper) {
        ...
        try {
            ...
            // spectral arrow: vanilla isInfinity == false (not Items.ARROW) -> mixin returns false -> consumed
    ...
    public void moreInfinityNegativePctAlwaysConsumes(TestContext helper) {
        ...
        try {
            Method m = RangedWeaponItem.class.getDeclaredMethod("getProjectile", ItemStack.class, ItemStack.class, LivingEntity.class, boolean.class);
            m.setAccessible(true);
            // threshold = clamp(1 - (-1)*1, 0, 1) = 1.0 -> deterministic never-free; loop for extra safety
            for (int i = 0; i < 20; i++) {
    ...
    public void moreBindingMultipleItemsRestoredToCorrectSlots(TestContext helper) {
        ...
        try {
            ...
            old.getInventory().dropAll();
            helper.assertTrue(old.getInventory().armor.get(3).isOf(Items.DIAMOND_HELMET)
            ...
            helper.assertTrue(head.isOf(Items.DIAMOND_HELMET)
                    && EnchantmentHelper.getLevel(Enchantments.BINDING_CURSE, head) == 5,
                "AFTER_RESPAWN should restore the helmet to head slot 3 (got " + head + ")");
            helper.assertTrue(feet.isOf(Items.DIAMOND_BOOTS)
                    && EnchantmentHelper.getLevel(Enchantments.BINDING_CURSE, feet) == 5,
                "AFTER_RESPAWN should restore the boots to feet slot 0 (got " + feet + ")");

            // second respawn: this player's stash entry was already removed -> nothing restored
            respawned.getInventory().armor.set(3, ItemStack.EMPTY);
            respawned.getInventory().armor.set(0, ItemStack.EMPTY);
            ServerPlayerEvents.AFTER_RESPAWN.invoker().afterRespawn(old, respawned, false);
            helper.assertTrue(respawned.getInventory().armor.get(3).isEmpty()
                    && respawned.getInventory().armor.get(0).isEmpty(),
                "A second AFTER_RESPAWN should restore nothing (stash entry already consumed)");
        } finally {
            ETTestHelper.setFeature("more_binding", false);
            ETTestHelper.setConfigValue("more_binding_step", "0.1");
