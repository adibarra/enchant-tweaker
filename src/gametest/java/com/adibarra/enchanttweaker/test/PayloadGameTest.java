...
public class PayloadGameTest implements FabricGameTest {
    ...
    // prime the capmod cache with cap 5, sync sharpness=3, and confirm the new cap
    // is read:
    ...
    public void restoreLocalConfigReloadsDisk(TestContext helper) {
        try {
            ...
            // `restoreLocalConfig` already reloaded the pristine disk state; re-run to be
            // certain
            ETMixinPlugin.restoreLocalConfig();
        }
        helper.complete();
    ...
    // a 2000-entry map forces a multi-byte writeVarInt(size) and a large body;
    // decode must
    ...
    public void payloadLargeMap(TestContext helper) {
        ...
        helper.assertTrue(decoded.configData().equals(big), "large map should round-trip exactly");
