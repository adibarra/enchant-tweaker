...
public class ETCommands {
    ...
    static {
        COMMANDS.add(new ADBrigadier.Command("Reloads the config file.",
            () -> CommandManager.literal("reload").executes(new ReloadCommand()).build()));

        COMMANDS.add(new ADBrigadier.Command("Check, set, list, or reset config keys.", () -> CommandManager
            .literal("config").executes(new ConfigCommand())
            // literal subcommands are matched before the <key> word argument
            .then(CommandManager.literal("list").executes(new ListCommand())
                .then(CommandManager.argument("category", StringArgumentType.word())
                    .suggests(ListCommand.CATEGORY_SUGGESTIONS).executes(new ListCommand())
                    .then(CommandManager.argument("page", IntegerArgumentType.integer(1)).executes(new ListCommand()))))
            .then(CommandManager.literal("reset").executes(new ResetCommand())
                .then(CommandManager.argument("key", StringArgumentType.word()).suggests(ResetCommand.KEY_SUGGESTIONS)
                    .executes(new ResetCommand())))
            .then(CommandManager.argument("key", StringArgumentType.word()).suggests(GetCommand.KEY_SUGGESTIONS)
                ...
                .then(CommandManager.argument("value", StringArgumentType.greedyString())
                    .suggests(ValueSuggestion.PROVIDER).executes(new SetCommand())))
            .build()));

        COMMANDS.add(new ADBrigadier.Command("Reports live runtime diagnostics.",
            () -> CommandManager.literal("diagnose").executes(new DiagnoseCommand()).build()));
    ...
    public static void registerCommands() {
        CommandRegistrationCallback.EVENT.register((commandDispatcher, registryAccess, environment) -> {
            CommandNode<ServerCommandSource> baseNode = CommandManager.literal(BASE_CMD)
                .requires(source -> source.hasPermissionLevel(2)).executes(new BaseCommand()).build();

            for (ADBrigadier.Command command : COMMANDS) {
                baseNode.addChild(command.node().get());
            }

            baseNode.addChild(CommandManager.literal("help").executes(new HelpCommand()).build());
